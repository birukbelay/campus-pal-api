...
import { RoleType } from './guards.dependencies';
...
export class JwtGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      const user: UserFromToken = (await this.jwtService.verifyAccessToken(
        token,
      )) as UserFromToken;
      ...
      const classRole = this.reflector.get<RoleType[]>(
        'roles',
        context.getClass(),
      );
      //roles defined by each handler function/resolver
      const handlerRole = this.reflector.get<RoleType[]>(
        'roles',
        context.getHandler(),
      );
